...
export default class uofStatusConfig {
    ...
    constructor(path: string) {
        ...
        if (typeof configObj.Logging.Db == "undefined")
            configObj.Logging.Db = {};
        if (typeof configObj.Logging.Api == "undefined")
            configObj.Logging.Api = {};
        if (typeof configObj.Logging.App == "undefined")
            configObj.Logging.App = {};
        if (typeof configObj.Logging.Timer === "undefined")
            configObj.Logging.Timer = {};
        this.config = {
            Service: {
            ...
            Api: {
                global_token:
                    configObj.Api.global_token ||
                    (function () {
                        var token = makeString();
                        console.warn(
                            "Service.global_token is not defined in the Configuration File"
                        );
                        console.warn("Auto-generated global_token: ", token);
                        console.warn(
                            "Please notice that this token will be discarded when the software stopped."
                        );
            ...
            Logging: {
                Api: {
                    ...
                    filename:
                        configObj.Logging.Api.filename ||
                        __dirname + "/logs/api.log",
                },
                Db: {
                    ...
                    filename:
                        configObj.Logging.Db.filename ||
                        __dirname + "/logs/db.log",
                },
                App: {
                    ...
                    filename:
                        configObj.Logging.App.filename ||
                ...
                Timer: {
                    ...
                    filename:
                        configObj.Logging.Timer.filename ||
                        __dirname + "/logs/timer.log",
                },
            },
            Timer: {
        ...
        log4js.configure({
            appenders: {
                f_db: {
                    type: "file",
                    filename: this.config.Logging.Db.filename,
                },
                f_app: {
                    type: "file",
                    filename: this.config.Logging.App.filename,
                },
                f_api: {
                    type: "file",
                    filename: this.config.Logging.Api.filename,
                },
                f_timer: {
                    type: "file",
                    filename: this.config.Logging.Timer.filename,
                },
            ...
            categories: {
                ...
                database: {
                    appenders: this.genAppender(this.config.Logging.Db, "db"),
                    level: this.config.Logging.Db.level,
                },
                api: {
                    appenders: this.genAppender(this.config.Logging.Api, "api"),
                    level: this.config.Logging.Api.level,
                },
                app: {
                    appenders: this.genAppender(this.config.Logging.App, "app"),
                    level: this.config.Logging.App.level,
                },
                timer: {
                    appenders: this.genAppender(
                        this.config.Logging.Timer,
                        "timer"
                    ),
                    level: this.config.Logging.Timer.level,
                },
            },
        });
    }
