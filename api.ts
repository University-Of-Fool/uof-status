            ...
            if (this.config.Api.global_token === Md5.hashStr(req.body.token)) {
                if (req.body.name) {
                    try {
                        ...
                        this.log.warn(
                            "Unknown error occupied in putServer:",
                            e
                        );
                    ...
                    this.log.info(
                        "put server failed, reason: No name provided"
                    );
                    res.status(400).send({
                        success: false,
        ...
        this.app.post("/api/status/put", async (req, res) => {
            try {
                ...
                if (typeof req.body.serverId === "number") {
                    var token = await this.db.queryServerToken(
                        req.body.serverId
                    );
                    ...
                    if (typeof req.body.online === "boolean") {
                        ...
                        this.log.info(
                            "put status failed, reason: no online provided"
                        );
                    }
                } else {
                    res.status(400).send({
                    ...
                    this.log.info(
                        "put status failed, reason: serverId undefined or NaN"
                    );
                }
            } catch (e) {
    ...
    private delServer() {
        this.app.post("/api/server/drop", async (req, res) => {
            try {
                ...
                this.log.debug("Body:" + JSON.stringify(req.body));
                if (
                    this.config.Api.global_token === Md5.hashStr(req.body.token)
                ) {
                    if (typeof req.body.serverId === "number") {
                        await this.db.delServer(req.body.serverId);
                        res.status(200).send({ success: true });
                        this.log.info(
                            "Server dropped with the id",
                            req.body.serverId
                        );
                    } else {
                        res.status(400).send({
                ...
                if (typeof e === "string")
                    ...
                    this.log.info(
                        "Known error PrismaClientKnownRequestError occpuied"
                    );
                else {
                    res.status(500).send({ error: JSON.stringify(e) });
                ...
                this.db
                    ...
                    .catch((e) => {
                        if (typeof e === "string") {
                            ...
                            res.status(500).send({ error: JSON.stringify(e) });
                            this.log.warn(
                                "Unknown error occupied in getStatus:",
                                e
                            );
                        }
                    });
