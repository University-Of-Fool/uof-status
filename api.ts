...
export default class uofStatusApi {
    ...
    private putServer() {
        this.app.post('/api/server/put', async (req, res) => {
            ...
            if (this.config.Api.global_token === Md5.hashStr(req.body.token)) {
                if (req.body.name) {
                    try {
                        var info = await this.db.newServer(req.body.name, req.body.description || "Server");
                        res.status(200).send({
                            success: true,
                            id: info.id,
                            token: info.token
                        });
                        this.log.info(`Server put with name ${req.body.name}, ${req.body.description}`);
                    } catch (e) {
                    ...
                    res.status(400).send({ success: false, reason: "No name provided" });
                }
            } else {
                this.log.info("put server failed, reason: Wrong token or no token provided")
                res.status(401).send({ success: false, reason: "Wrong token or no token provided" });
            }
        });
    }
    private putStatus() {
            ...
            try {
                ...
                if (typeof (req.body.serverId) === "number") {
                    ...
                    if (token !== Md5.hashStr(req.body.token)) {
                        this.log.info("put status failed, reason: Wrong token or no token provided")
                        res.status(401).send({ success: false, reason: "Wrong token or no token provided" });
                        return;
                    }
                    if (typeof (req.body.online) === "boolean") {
                        ...
                        this.log.info(`Status put with the id ${req.body.serverId} status ${req.body.online}`)
                    } else {
                        res.status(400).send({ success: false, reason: "no online provided" });
                        this.log.info("put status failed, reason: no online provided")
                    }
                } else {
                    res.status(400).send({ success: false, reason: "serverId undefined or NaN" });
            ...
            try {
                ...
                if (this.config.Api.global_token === Md5.hashStr(req.body.token)) {
                    if (typeof (req.body.serverId) === "number") {
                        ...
                        res.status(400).send({ success: false, reason: "serverId undefined or NaN" });
                        this.log.info("Drop server failed, reason: serverId undefined or NaN")
                    }
                } else {
                    res.status(401).send({ success: false, reason: "Wrong token or no token provided" });
                    this.log.info("Drop server failed, reason: Wrong token or no token provided")
                }
            } catch (e: any) {
                if (typeof e === 'string') res.status(404).send({ success: false, reason: e });
                else if (e.constructor.name == "PrismaClientKnownRequestError") this.log.info("Known error PrismaClientKnownRequestError occpuied");
                else {
                    res.status(500).send({ error: JSON.stringify(e) });
    ...
    private getStatus() {
        this.app.get('/api/status/get/:id', (req, res) => {
            ...
            try {
                this.db.queryLatestStatus(Number(req.params.id)).then(value => {
                    var _value:any;
                    if(!value) _value={};
                    else _value=value;
                    res.status(200).send({ success: true, serverId: _value.serverId||req.params.id, status: (()=>{if(typeof _value.status==="boolean"){return _value.status}else{return "Undefined";}})(), time: _value.time||"Undefined" });
    ...
    constructor(express: Express, db: uofStatusDatabase, config: Configuration, blocked?: boolean) {
        this.app = express;
        this.db = db;
        this.config = config;
