...
program.version('0.0.1')
   .option('-c --configuration <filename>', 'configuration file path', __dirname + '/config.toml')
...
var timer = new uofStatusTimer(db, config.getConfig().Timer.interval, config.getConfig().Timer.max_seconds);
...
app.listen(config.getConfig().Service.port, config.getConfig().Service.listen, () => { 
   log2c.mark(`Server started at http://${config.getConfig().Service.listen}:${config.getConfig().Service.port}`);
});
